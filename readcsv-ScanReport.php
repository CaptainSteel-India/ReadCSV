...
$mainloop = 0;
$datefile = date('Ymd');
$fname = '/home/ec2-user/csv/'.$datefile.'-ScanReport.csv';
//$fname = '/home/ec2-user/csv/20231113-ScanReport.csv';
...
if (($open = fopen($fname, "r")) !== false) {
  ...
  fwrite($logfile, "ERROR! Could not open the CSV file for Champion Scannings ERROR!". PHP_EOL); 
      ...
      //data sanitisation
 //data insert
 $insert_sql = "INSERT INTO `tbl_champion_scan`(`firstname`, `lastname`,
  `mobile`, `city`, `state`, `scandate`, `qrcode`, `multiplier`,
   `statecampaignstatus`, `bonusmultiplier`, `campaignstart`, `campaignend`,
    `finalvalue`, `latitude`, `longitude`) VALUES ";
  foreach($array as $arrays){
    if ($arrays[6]!="")
      {
      $scandate = "'". substr($arrays[6], 6, 4). "-". substr($arrays[6], 0, 2). "-". substr($arrays[6], 3, 2). " ". substr($arrays[6], 11, 8) . "'";
      }
    else
      {
        $scandate = 'null';
...
if($arrays[1]!=""){ $firstname=rtrim($arrays[1]);} else {  $firstname='null';}

if($arrays[2]!=""){  $lastname=rtrim($arrays[2]);} else {  $lastname='null';}
...
$insertvalue=$insertvalue .PHP_EOL. " ('". $firstname. "','" . $lastname. "','" . $arrays[3]. "','" . $arrays[4] .
 "','" . $arrays[5] . "'," . $scandate .",'" . $arrays[7] ."'," . $arrays[8] . "," . $statestatus .
 "," . $bonusmultiplier . "," . $campaignstart."," . $campaignend. "," . $arrays[13]. ",'" .$arrays[15].
 "','" .$arrays[14] ."')";
//echo $insertvalue;
//echo PHP_EOL;
$insertcount++;
